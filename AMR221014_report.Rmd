...
res_tbl <- read_csv("./data_output/res_tbl.csv") %>% filter(!is.na(ENTREZID), !duplicated(gene, drop = FALSE))
diff <- read_csv("./data_output/sign_different.csv")
res_tbl1 <-  res_tbl %>% filter(!is.na(ENTREZID), padj<0.05,
                                !duplicated(gene, drop = FALSE))
...
res_tbl %>%
  filter(!is.na(padj),!duplicated(gene, drop = FALSE)) %>%
...
## avec les points en bleu et rouge

outliers = res_tbl %>% 
  filter(padj < 0.05 & log2FoldChange<=-1|padj < 0.05 &log2FoldChange>=1)

up = res_tbl %>% filter(padj <= 0.05 & log2FoldChange >= 1)
down = res_tbl %>% filter(padj <= 0.05 & log2FoldChange <= -1)

res_tbl %>%
  filter(!is.na(padj),!duplicated(gene, drop = FALSE)) %>%
  ggplot(aes(x = log2FoldChange, y = -log10(padj),label = gene)) +
  geom_point(size = 0.5, color = "gray") +
  geom_point(data = res_tbl %>% filter(padj <= 0.05 & log2FoldChange >= 1), size= 0.5, color = "firebrick3")+
  geom_point(data = res_tbl %>% filter(padj <= 0.05 & log2FoldChange <= -1), size= 0.5, color = "blue")+
  geom_hline(yintercept = -log10(0.05)) +
  geom_vline(xintercept = 1) +
  geom_vline(xintercept = -1)+
  geom_label_repel(data = subset(res_tbl, gene%in%up$gene),
                  box.padding   = 0.35,
                  point.padding = 0.5,
                  color = "firebrick3",
                  segment.color = 'grey50',
                  max.overlaps = 30)+
  geom_label_repel(data = subset(res_tbl, gene%in%down$gene),
                  box.padding   = 0.35,
                  point.padding = 0.5,
                  color = "blue",
                  segment.color = 'grey50',
...
## Pour label que tes points d'intérêts, utilise le code ci-dessous

targetup = c("ELF1", "PDCD4")
targetdown = c("NDUFA3")
...
res_tbl %>%
  ...
  ggplot(aes(x = log2FoldChange, y = -log10(padj),label = gene)) +
  geom_point(size = 0.5, color = "gray") +
  geom_point(data = up, size= 0.5, color = "firebrick3")+
  geom_point(data = down, size= 0.5, color = "blue")+
  geom_hline(yintercept = -log10(0.05)) +
  geom_vline(xintercept = 1) +
  geom_vline(xintercept = -1)+
  geom_label_repel(data = subset(up, gene%in% targetup),
                  box.padding   = 0.35,
                  color = "firebrick3",
                  point.padding = 0.5,
                  segment.color = 'grey50',
                  max.overlaps = 30)+
  geom_label_repel(data = subset(down, gene%in% targetdown),
                  box.padding   = 0.35,
                  color = "blue",
                  point.padding = 0.5,
                  segment.color = 'grey50',
...
```{r Epith Msigdb query, fig.height = 6, fig.width = 6}
...
a) la machinerie UPR
b) les familles d’oxydo-réduction (enzymes NOX, TXN, reduction du glutathion…)
c) Machinerie de presentation (CRT, TAP; proteasome…)

### The antigen presenting pathway (GO:0019882)

```{r}

GO_ag = read_tsv("./data/GO0019882.tsv", col_names = F)
colnames(GO_ag) = c("gene","uniprot","description","X1","GO_term",
...
target = res_tbl1 %>% 
  filter(gene%in%GO_ag$gene, ignore.case = TRUE)
...
GO_upr = read_tsv("./data/GO0030968_upr.txt")
GO_upr$Symbol_up = toupper(GO_upr$Symbol)
...
target = res_tbl1 %>% 
  filter(gene%in%GO_upr$Symbol_up)
...
GO_redox = read_tsv("./data/GO001649_redox.txt")
GO_redox$Symbol_up = toupper(GO_redox$Symbol)
...
target = res_tbl1 %>% 
  filter(gene%in%GO_redox$Symbol_up)
