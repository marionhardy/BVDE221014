...
counts <- as.data.frame(read.csv("./data/AMR221014_counts.csv", row.names = 1))
...
p2 <- plotPCA(rld,intgroup="dates") 
p3 <- plotPCA(rld,intgroup=c("condition","dates")) # determine which samples to relabel
...
rownames(sampleDistMatrix) <- paste(dds$condition, dds$sample, sep="-")
...
## H2O2 vs ctrl treatment ------------------------------------------------------

res <- results(dds, name = "condition_H2O2_vs_ctrl")
res_tbl <- as_tibble(res, rownames="ensembl")

# Import annotation file

ensembl_to_geneName <- readRDS("./data/hsapiens_annotations_230505.rds")

# Add gene names and entrez id to your results

res_tbl <- res_tbl %>%
  left_join(ensembl_to_geneName, by = c("ensembl"="ensembl_version")) %>%
  arrange(padj)

write.csv(res_tbl,"./data_output/res_tbl.csv", row.names = T)
...
write.xlsx(list_of_datasets, file = "./data_output/AMR221014_sign_genes.xlsx")
...
ggsave("./figures/volcanoplot", last_plot(), device = png, dpi= 500)
...
saveRDS(dds, file = "./data_output/dds.rds")
saveRDS(res_tbl, file = "./data_output/res_tbl.rds")
write_csv(res_tbl, file = "./data_output/res_tbl.csv")
res_tbl1 <-  res_tbl %>% filter(!is.na(ENTREZID), padj<0.05)
saveRDS(res_tbl1, file = "./data_output/res_tbl_signif.rds")
write_csv(res_tbl1, file = "./data_output/res_tbl_signif.csv")
